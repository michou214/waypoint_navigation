...
#define MODE 1
#define MODE_T5L 0      // Take off, stay 5 secondes in the air, lands
...
("tag_detections", 10, APtag_est_pos_cb);
    ...
    if(MODE == MODE_TPL)
        for(int p=0; p<size_wp; p++){
            waypoints[p](0) =  1.0f + 2.0f*p;
            waypoints[p](1) = -1.0f + 2.0f*p;
    ...
    while(ros::ok()){
        ...
        if(!skip && AP_detected && !AP_verified){
            ...
            if(arming_client.call(arm_cmd) && is_goal_reached(goal_pos, pos, POS_ACCEPT_RAD)){
                ...
                else if(takeoff_done && MODE == MODE_T5L && !landing_in_progress){
                    traj_done = true;
